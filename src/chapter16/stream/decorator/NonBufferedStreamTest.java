...
//버퍼기능을 제공하는 보조스트림을 사용하지 않을 경우 속도 체크
...
public class NonBufferedStreamTest {
	...
	public static void main(String[] args) {
		...
		try(
				FileInputStream fis = new FileInputStream("a.zip"); //"a.zip" 준비가 되어있어야함
				FileOutputStream fos = new FileOutputStream("copy.zip") // 객체생성이 되면, 자동으로 "copy.zip"파일이 생성
						
			)
		{
			//파일 복사 작업이 시작하기 전 1970년 1월 1일 00시 00초에서 
			//현재 날짜와 시간까지 흐른 누적된 시간을 밀리세컨드로 반환해준다.
			milisecond = System.currentTimeMillis();
			
			//파일 복사. 입력스트림(fis)으로 "a.zip"파일의 내용을 읽어와서, 출력스트림(fos)으로 "copy.zip"파일에 내용을 쓰는 작업
			int i;
			while((i=fis.read()) != -1) {
				fos.write(i);
			}
